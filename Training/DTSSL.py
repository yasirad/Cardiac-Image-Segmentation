...
def temp_sced(t): #cosine schedule
    tplus = 1
    tminus = 0.07
    T = 200
    cosineSched = (tplus-tminus)*(1+math.cos(2*math.pi*t/T))/2+tminus
    return cosineSched

def sims(x,y,i,temperature):
...
def simcl(x,y,t):
    ...
    for i in range(batchSize):
        globalL =globalL+ sims(x,y,i,temp)
...
for epoch in range(epochs):
    ...
    for i in range(len(training)):
        ...
        with tf.GradientTape() as tape:  
            ...
            reg_flat1 = proj(enc1)
            reg_flat2 = proj(enc2)
            l = simcl(reg_flat1, reg_flat2,epoch) #add epoch for temp
            loss.append(l)
